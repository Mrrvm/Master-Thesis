...
\begin{align}
	...
	\lambda_1 \mathbf{\tilde{m}^r_1} =  K^{-1} [ R^{-1} \ \mathbf{-t} ]K  \lambda_1 \mathbf{\tilde{m}_2}
\end{align}
\begin{equation}
...
Once the fundamental matrix is obtained through one of those methods, assuming the intrinsic parameters are known, which is the case for this project, the essential matrix may be obtained by $E = K^{T}  F K$ as seen on (\ref{hhh}).
